...
class Experiment:
    def __init__(self, num_iterations, batch_size, lr, dr, input_drop, hidden_drop, feature_drop,
                 RAD_Size, num_heads, dembd, dembd1, max_ary):          
    ...
    def train_and_eval(self):
        ...
        model = HyMRformer(len(d.ent2id), len(d.rel2id), self.input_drop, self.hidden_drop, self.feature_drop,
                          self.RAD_Size, self.num_heads,
                          self.dembd, self.dembd1, self.max_ary, self.device)
        model.to(device)
            ...
            if it % param['eval_step'] == 0:
                ...
                with torch.no_grad():
                    ...
                    if it - best_valid_iter >= param['valid_patience'] or it == self.num_iterations:
                        ...
                        # # print(mrr_lst)
                        # with open("WPmrr_10iter_Wi.txt", "w") as f:
                        #     for mrr in mrr_lst:
                        #         f.write(str(mrr) + ",")
                        #     f.close()

                        # with open("WPhit1_10iter_Wi.txt", "w") as f:
                        #     for hit1 in hit1_lst:
                        #         f.write(str(hit1) + ",")
                        #     f.close()
                        # with open("WPhit3_10iter_Wi.txt", "w") as f:
                        #     for hit3 in hit3_lst:
                        #         f.write(str(hit3) + ",")
                        #     f.close()
                        # with open("WPhit10_10iter_Wi.txt", "w") as f:
                        #     for hit10 in hit10_lst:
                        #         f.write(str(hit10) + ",")
                        #     f.close()
                        # with open("WPlosses_10iter_Wi.txt", "w") as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--RAD_Size", type=int, default=6, nargs="?", help="The size coefficient.")
    parser.add_argument("--num_heads", type=int, default=4, nargs="?", help="The head number of Attention.")
    
    parser.add_argument("--dembed", type=int, default=400 , nargs="?")
    parser.add_argument("--dembed1", type=int, default=10, nargs="?")
    ...
    Exp = Experiment(num_iterations=param['num_iterations'], batch_size=param['batch_size'], lr=param['lr'], dr=param['dr'],
                     ...
                     RAD_Size=param['RAD_Size'], num_heads=param['num_heads'],
